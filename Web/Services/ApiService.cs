...
public class ApiService
{
   ...
   public async Task<List<ProjectsDTO>> GetAllProjectAsync()
    ...
    //############################################### DesignPhase Services ##################################################

    public async Task<DesignPhaseDTO?> GetDesignPhasePhaseByIdAsync(int id)
    {
        await AddAuthHeaderAsync(); // Optional if your endpoint requires JWT auth

        var response = await http.GetAsync($"api/design/{id}");

        if (response.IsSuccessStatusCode)
        {
            return await response.Content.ReadFromJsonAsync<DesignPhaseDTO>();
        }
        else
        {
            var error = await response.Content.ReadAsStringAsync();
    ...
    // ✅ ADD OR UPDATE DESIGN (Calling the API)
    public async Task<DesignPhaseDTO> AddOrUpdateDesignAsync(DesignPhaseDTO designPhaseDTO)
    {
        await AddAuthHeaderAsync();

        // Send POST request to the API with the DesignPhaseDTO payload
        var response = await http.PostAsJsonAsync("api/design/add-or-update", designPhaseDTO);

        if (response.IsSuccessStatusCode)
        {
            return await response.Content.ReadFromJsonAsync<DesignPhaseDTO>();  // Successful request, return the response
        }
        else
        {
            var error = await response.Content.ReadAsStringAsync();
    ...
    //############################################### DevelopmentPhase Services ##################################################

    public async Task<DevelopmentPhaseDTO?> GetDevelopmentPhaseByIdAsync(int id)
    {
        await AddAuthHeaderAsync(); // Optional if your endpoint requires JWT auth

        var response = await http.GetAsync($"api/dev/{id}");

        if (response.IsSuccessStatusCode)
        {
            return await response.Content.ReadFromJsonAsync<DevelopmentPhaseDTO>();
        }
        else
        {
            var error = await response.Content.ReadAsStringAsync();
    ...
    // ✅ ADD OR UPDATE DEVELOPMENT (Calling the API)

    public async Task<DevelopmentPhaseDTO?> AddOrUpdateDevelopmentPhaseAsync(DevelopmentPhaseDTO developmentPhaseDTO)
    {
        await AddAuthHeaderAsync();

        var response = await http.PostAsJsonAsync("api/dev/add-or-update", developmentPhaseDTO);
    ...
    //############################################### MaintanencePhase Services ##################################################

    public async Task<MaintenancePhaseDTO?> GetMaintenancePhaseByIdAsync(int id)
    {
        await AddAuthHeaderAsync(); // Optional if your endpoint requires JWT auth

        var response = await http.GetAsync($"api/maintanence/{id}");

        if (response.IsSuccessStatusCode)
        {
            return await response.Content.ReadFromJsonAsync<MaintenancePhaseDTO>();
        }
        else
        {
            var error = await response.Content.ReadAsStringAsync();
    ...
    // ✅ ADD OR UPDATE MAINTANANCE (Calling the API)
    public async Task<MaintenancePhaseDTO> AddOrUpdateMaintenancePhaseAsync(MaintenancePhaseDTO maintenancePhaseDTO)
    {
        await AddAuthHeaderAsync();

        // Send POST request to the API with the MaintenancePhaseDTO payload
        var response = await http.PostAsJsonAsync("api/maintanence/add-or-update", maintenancePhaseDTO);

        if (response.IsSuccessStatusCode)
        {
            return await response.Content.ReadFromJsonAsync<MaintenancePhaseDTO>(); ; // Successful request, return the response
        }
        else
        {
            var error = await response.Content.ReadAsStringAsync();
